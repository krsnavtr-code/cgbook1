...
app.use((req, res, next) => {
  ...
  res.setHeader(
    "Access-Control-Allow-Headers",
    "Origin, X-Requested-With, Content-Type, Accept, Authorization, x-auth-token, x-user-agent, x-client-ip",
  );
...
app.get("/test-upload/:filename", (req, res) => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    res
      .status(404)
      .json({ success: false, message: "File not found", path: filePath });
  }
});
