...
// @access  Private
router.put("/file/:filename", protect, async (req, res) => {
  try {
    const { filename } = req.params;
    ...
    // Validate new filename (prevent directory traversal and invalid characters)
    if (
    ...
    // Construct URLs for the renamed file
    const fileUrl = `/upload/file/${encodeURIComponent(newFilename)}`;
    const fullUrl = `${req.protocol}://${req.get("host")}${fileUrl}`;
